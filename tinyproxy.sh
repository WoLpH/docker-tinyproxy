...
displayUsage() {
    echo
    echo '  Usage:'
    echo "      docker run -d --name='tinyproxy' -p <Host_Port>:8888 wolph/tinyproxy:latest <ACL>"
    echo
    ...
    echo
    echo "      Examples:"
    echo "          docker run -d --name='tinyproxy' -p 6666:8888 wolph/tinyproxy:latest ANY"
    echo "          docker run -d --name='tinyproxy' -p 7777:8888 wolph/tinyproxy:latest 87.115.60.124"
    echo "          docker run -d --name='tinyproxy' -p 8888:8888 wolph/tinyproxy:latest 10.103.0.100/24 192.168.1.22/16"
    echo
}
